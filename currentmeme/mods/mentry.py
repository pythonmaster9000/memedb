...
class Entry:
    ...
    def save_to_folder(self):
        if not self.filepath:
            ...
            return False
        if re.search("^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+", self.attachment_url):
            ydl_opts = {'format': 'mp4',
                        'outtmpl': fr'{self.filepath}/{self.filename}.mp4'}
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download(self.attachment_url)
        else:
            try:
        ...
        if self.Compress.check_size() < 8:
            return True
        elif self.Compress.check_size() < 16:
