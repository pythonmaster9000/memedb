...
class Compress:
    ...
    def check_size(self):
        return int(
            ffmpeg.probe(fr'{self.filepath}\{self.filename}.mp4', select_streams="v")['format']['size']) / 1048576
    ...
    def compress(self):
        ...
        subprocess.run(f'ffmpeg -y -i {self.filepath}\{self.filename}.mp4 -b 800k {self.filepath}\{self.filename}1.mp4')
